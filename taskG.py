...
# Build model
# -----------------------------
def build_network(gas_case="CH4", keep_battery=True):
    ...
    n.add(
        ...
        p_max_pu=get_hourly_values(data_wind, "CHE", snapshots),
        capital_cost=costs.at["solar", "capital_cost"],
        marginal_cost=costs.at["solar", "marginal_cost"],
        p_nom_extendable=True,
    )
...
(summary_df.set_index("gas_case")["objective_EUR"] / 1e9).plot(kind="bar", ax=axes[0])
axes[0].set_title("Total system cost")
axes[0].set_ylabel("Billion EUR")

summary_df.set_index("gas_case")["pipeline_capacity_MW_each"].plot(kind="bar", ax=axes[1])
axes[1].set_title("Pipeline capacity per corridor")
axes[1].set_ylabel("MW_th")

comparison_transport = summary_df.set_index("gas_case")[["electricity_line_transport_TWh", "gas_pipeline_transport_TWh"]]
comparison_transport.plot(kind="bar", ax=axes[2])
axes[2].set_title("Transported energy comparison")
...
fig.tight_layout()
fig.savefig(os.path.join(RESULTS_DIR, "final_comparison.png"), dpi=150)
plt.show()
